How I Use the Advantage360 Pro
==============================

<div class="center">[Esperanto](/eo/adv360/) • English</div>
<div class="center">Last updated: January 17, 2024</div>
...
Advantage and [Kinesis Advantage360 Professional](https://kinesis-ergo.com/keyboards/advantage360/). I previously
wrote about the [Kinesis Advantage](/en/advantage/) and praised it.

When I learned that Kinesis is going to introduce a new line of Advantage
keyboards with the ability to remap the keys using software, and it has wireless
connectivity, I became curious. I contacted my Kinesis supplier from Japan, the
same guy that I got my Advantage from. I got a reply within 24 hours and he said
that they indeed have the Adv360 Pro on stock. I ordered it. I was stoked.

In came in by mail within two months. Now, that amount of time may seem long,
but for a premium product like it, it was worth it.

After I heard the announcement of the new keyboard model came, I did extensive
research about it, learning everything that I can from it. I learned that it
uses the [ZMK firmware](https://zmk.dev).  I learned that in order to re-define the keys, you use a
...
Of course, I wasn't happy with the default layout and went on to change it.
First, was to go to the
[Adv360-Pro-ZMK](https://github.com/KinesisCorporation/Adv360-Pro-ZMK)
repository, then fork it. Next is to enable Actions so that every time you push
changes to your repository, new firmware files are going to be built.

So, what I would do was make some changes in the web app, save the changes, then
wait for the firmware files to be built. The resulting files are `left.uf2` and
`right.uf2` for the left and right sides, respectively.
...
<kbd>End</kbd> keys. I have always used the latter.
...
discovered, that it's actually straightforward. The [official ZMK
documentation](https://zmk.dev/docs) is very helpful, here.
...
`.dtsi` is short for Device Tree Structrure Includes, which is a type of an
include file for DTS configuration.


<a name="modmorphs">Mod-morphs</a>
----------------------------------
...
<kbd>Shift+9</kbd> would yould a `/`. The same with `0` and `>`.
...
<kbd>Mod1+`<kbd> input six graves, the put the cursor in the middle. I use this
whenever I need to type Markdown code blocks.

To do so, I put the following ing `macros.dtsi`:
...
To use `n9_less` and `n0_greater`, you use `&n9_less` and `&n0_greater`, instead
of `&kp 9` and `&kp 0`, respectively, in `adv360.keymap.`. They appear as such
in the
[adv360.keymap](https://github.com/ebzzry/adv360-pro-zmk/blob/V3.0/config/adv360.keymap#L21)
file.
...
keyboard with had to have a translation layer. I thought, why not just have the
keyboard input Dvorak keys, instead. So, that's what I did. I changed the
definitions of all the keys so that they would emit Dvorak keys. I should have
done that a long time ago.

For the main layer, the `adv360.keymap` on the left half of the keyboard is,
...
When I need to update the configuration, I would modify the aforementioned
files, then push the changes to GitHub, wait for the firmware files to be built,
then flash them to the keyboard.
...
manageable. Overall, I would say, that the Adv360 is a top-tier input device
that has secured its own spot in the rich world of (mechanical) keyboards.
